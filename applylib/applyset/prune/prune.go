...
func (a *ApplySet) FindAllObjectsToPrune(ctx context.Context, dynamicClient dynamic.Interface, visitedUids sets.Set[types.UID]) ([]PruneObject, error) {
	...
	for _, gk := range a.AllPrunableResources() {
		restMappings, err := a.restMapper.RESTMappings(gk)
		if err != nil {
			return nil, fmt.Errorf("cannot get restmapping for %v: %w", gk, err)
		}
		if len(restMappings) == 0 {
			return nil, fmt.Errorf("no restmappings for %v: %w", gk, err)
		}
		restMapping := restMappings[0]
...
func (a *ApplySet) deleteObjects(ctx context.Context, dynamicClient dynamic.Interface, pruneObjects []PruneObject, opt *ApplySetDeleteOptions) error {
	for i := range pruneObjects {
		...
		if err := runDelete(ctx, namespace, name, mapping, dynamicClient, opt.CascadingStrategy, opt.GracePeriod, opt.DryRun); err != nil {
